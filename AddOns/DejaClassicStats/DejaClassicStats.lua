...
local loader = CreateFrame("Frame")
	...
	loader:SetScript("OnEvent", function(self, event, arg1)
		if event == "ADDON_LOADED" and arg1 == "DejaClassicStats" then
			...
			DejaClassicStatsDB = initDB(DejaClassicStatsDB, gdbprivate.gdbdefaults) --the first per account saved variable. The second per-account variable DCS_ClassSpecDB is handled in DCS_Layouts.lua
			gdbprivate.gdb = DejaClassicStatsDB --fast access for checkbox states
...
local loader = CreateFrame("Frame")
	...
	loader:SetScript("OnEvent", function(self, event, arg1)
		if event == "ADDON_LOADED" and arg1 == "DejaClassicStats" then
			...
			DejaClassicStatsDBPC = initDB(DejaClassicStatsDBPC, private.defaults) --saved variable per character, currently not used.
			private.db = DejaClassicStatsDBPC
...
function DejaClassicStats.SlashCmdHandler(msg, editbox)
	...
	elseif (msg == "reset") then
		--DejaClassicStatsDBPC = private.defaults
...
local StatScrollFrame = CreateFrame("ScrollFrame", "StatScrollFrame", CharacterFrame, "UIPanelScrollFrameTemplate")
	StatScrollFrame:ClearAllPoints()
	StatScrollFrame:SetSize( DCS_HeaderWidth, 408 )
	StatScrollFrame:SetPoint("TOPLEFT", "CharacterFrame", "TOPRIGHT", -34, -22) -- This is (-40, -14) for Classic, different for dry development
	StatScrollFrame:SetFrameStrata("BACKGROUND")
	StatScrollFrame.ScrollBar:ClearAllPoints()
	StatScrollFrame.ScrollBar:SetPoint("TOPLEFT", StatScrollFrame, "TOPRIGHT", 0, -16)
	StatScrollFrame.ScrollBar:SetPoint("BOTTOMLEFT", StatScrollFrame, "BOTTOMRIGHT", 0, 16)
	-- StatScrollFrame.ScrollBar:Hide() -- This will not hide the ScrollBar if the "OnScrollRangeChanged" script has a SetShown and is not hidden. 
	
	local t=StatScrollFrame:CreateTexture(nil,"ARTWORK")
	t:SetAllPoints(StatScrollFrame)
	t:SetColorTexture(0, 0, 0, 1)

	local DCS_TopTexture=StatScrollFrame:CreateTexture(nil,"ARTWORK")
	DCS_TopTexture:SetPoint("TOPLEFT", StatScrollFrame, "TOPLEFT", -4, 82)
	...
	local DCS_TopRightTexture=StatScrollFrame:CreateTexture(nil,"ARTWORK")
	DCS_TopRightTexture:SetPoint("TOPRIGHT", StatScrollFrame, "TOPRIGHT", 60, 82)
	...
	local DCS_LeftTexture=StatScrollFrame:CreateTexture(nil,"ARTWORK")
	DCS_LeftTexture:SetPoint("LEFT", StatScrollFrame, "LEFT", 0, -20)
	...
	local DCS_RightTexture=StatScrollFrame:CreateTexture(nil,"ARTWORK")
	DCS_RightTexture:SetPoint("RIGHT", StatScrollFrame, "RIGHT", 60, 0)
	...
	local DCS_BottomRightTexture=StatScrollFrame:CreateTexture(nil,"ARTWORK")
	DCS_BottomRightTexture:SetPoint("BOTTOMRIGHT", StatScrollFrame, "BOTTOMRIGHT", 60, -82)
	DCS_BottomRightTexture:SetTexture("Interface\\PaperDollInfoFrame\\UI-Character-General-BottomRight")

	local DCS_BottomTexture=StatScrollFrame:CreateTexture(nil,"ARTWORK")
	DCS_BottomTexture:SetPoint("BOTTOMLEFT", StatScrollFrame, "BOTTOMLEFT", -4, -82)
	...
	local t=StatScrollFrame.ScrollBar:CreateTexture(nil,"ARTWORK")
		t:SetAllPoints(StatScrollFrame.ScrollBar)
		t:SetColorTexture(0, 0, 0, 1)
...
local DejaClassicStatsPane = CreateFrame("Frame", "DejaClassicStatsPane", CharacterFrame)
	...
	DejaClassicStatsPane:SetScript("OnEvent", function(self, event, ...)
		...
		DejaClassicStatsPane:SetAllPoints("StatScrollFrame") -- This is (-40, -14) for Classic, different for dry development
		-- DejaClassicStatsPane:SetFrameStrata("BACKGROUND")
		DejaClassicStatsPane:Show()

		StatScrollFrame:SetScrollChild(DejaClassicStatsPane)
	end)
...
local DCSPrimaryStatsFS = DCSPrimaryStatsHeader:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	DCSPrimaryStatsFS:SetText('Primary')
...
local DCSOffenseStatsFS = DCSOffenseStatsHeader:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	DCSOffenseStatsFS:SetText('Offense')
...
local DCSDefenseStatsFS = DCSDefenseStatsHeader:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	DCSDefenseStatsFS:SetText('Defense')
...
local DCSPrimaryStatsFS = DCSMeleeEnhancementsStatsHeader:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	DCSPrimaryStatsFS:SetText('Melee Enhancements')
	DCSPrimaryStatsFS:SetTextColor(1, 1, 1)
	DCSPrimaryStatsFS:SetPoint("CENTER", 0, 0)
	DCSPrimaryStatsFS:SetFont("Fonts\\FRIZQT__.TTF", 12, "THINOUTLINE")
	DCSPrimaryStatsFS:SetJustifyH("CENTER")
...
local DCSPrimaryStatsFS = DCSSpellEnhancementsStatsHeader:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	DCSPrimaryStatsFS:SetText('Spell Enhancements')
	DCSPrimaryStatsFS:SetTextColor(1, 1, 1)
	DCSPrimaryStatsFS:SetPoint("CENTER", 0, 0)
	DCSPrimaryStatsFS:SetFont("Fonts\\FRIZQT__.TTF", 12, "THINOUTLINE")
	DCSPrimaryStatsFS:SetJustifyH("CENTER")
...
local function MovementSpeed()
    ...
    return math.floor( (playerSpeed / 7) * 100) .. "%"
end
---------------------------
...
DCS_STAT_DATA = {
	...
	-- 	StatValue = 0,
	-- 	isShown = true,
	-- 	Label = "Physical Critical Strike: ",	
	...
	-- 	isShown = true,
	-- 	Label = "Mana Regen Current: ",	
	-- 	statFunction = ManaRegenCurrent,
	-- 	relativeTo = DCSSpellEnhancementsStatsHeader,
	-- },
	ManaRegenNotCasting = {
		isShown = true,
		Label = "Mana Regen Not Casting: ",	
		statFunction = ManaRegenNotCasting,
		relativeTo = DCSSpellEnhancementsStatsHeader,
	},
	ManaRegenCasting = {
		isShown = true,
		Label = "Mana Regen Casting: ",	
		statFunction = ManaRegenCasting,
		relativeTo = DCSSpellEnhancementsStatsHeader,
	},
	SpellCritChance = {
		isShown = true,
		Label = "Spell Critical Strike: ",	
...
local function DCS_CreateStatText(StatKey, StatValue, XoffSet, YoffSet)
	DejaClassicStatsPane.stat = DejaClassicStatsPane:CreateFontString(StatKey.."NameFS", "GameFontNormal")
	DejaClassicStatsPane.stat:SetPoint("TOPLEFT", DCS_STAT_DATA[StatKey].relativeTo, "BOTTOMLEFT", (15 + XoffSet), (-14 * (YoffSet - 1)))
	DejaClassicStatsPane.stat:SetFont("Fonts\\FRIZQT__.TTF", 12)
	...
	DejaClassicStatsPane.value = DejaClassicStatsPane:CreateFontString(StatKey.."ValueFS", "GameFontNormal")
	DejaClassicStatsPane.value:SetPoint("TOPRIGHT", DCS_STAT_DATA[StatKey].relativeTo, "BOTTOMRIGHT", (-16), (-14 * (YoffSet - 1)))
	DejaClassicStatsPane.value:SetFont("Fonts\\FRIZQT__.TTF", 12)
...
local function DCS_SetStatText(StatKey, StatValue, XoffSet, YoffSet)
	_G[StatKey.."NameFS"]:SetText(DCS_STAT_DATA[StatKey].Label)
	_G[StatKey.."ValueFS"]:SetText(StatValue)
end
...
local function DCS_SET_STATS_TEXT()
	for k, v in ipairs(DCS_PRIMARY_STAT_LIST) do
		DCS_STAT_DATA[v].StatValue = DCS_STAT_DATA[v].statFunction()
		DCS_SetStatText(v, DCS_STAT_DATA[v].StatValue, 0, 0)
	end
	for k, v in ipairs(DCS_MELEE_STAT_LIST) do
		DCS_STAT_DATA[v].StatValue = DCS_STAT_DATA[v].statFunction()
		DCS_SetStatText(v, DCS_STAT_DATA[v].StatValue, 0, 0)
...
local function DCS_TalentArtFrames(v, frameTL, frameTR, frameBL, frameBR, drawLayer, DCS_TalentSpec, frameTLrelativeTo, relativePoint)
	...
	if (frameexists) then --Check for first frame, assume others unless errors occur
		...
		frameTL:SetTexture("Interface\\TALENTFRAME\\"..DCS_CLASSIC_SPECS[className].spec["tree"..DCS_TalentSpec].."-TopLeft")
		...
		frameTR:SetTexture("Interface\\TALENTFRAME\\"..DCS_CLASSIC_SPECS[className].spec["tree"..DCS_TalentSpec].."-TopRight")
		...
		frameBL:SetTexture("Interface\\TALENTFRAME\\"..DCS_CLASSIC_SPECS[className].spec["tree"..DCS_TalentSpec].."-BottomLeft")
		...
		frameBR:SetTexture("Interface\\TALENTFRAME\\"..DCS_CLASSIC_SPECS[className].spec["tree"..DCS_TalentSpec].."-BottomRight")
